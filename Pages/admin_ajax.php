...
require_once '../Inc/Constants/functions.php';

const DATE_FORMAT_HIDDEN = "d/m/Y H:i";
const ERR_INVALID_DATA = "Données invalides";
const ERR_INVALID_ROLE = "Rôle invalide";
const ERR_INVALID_STATUS = "Statut invalide";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_csrf_token($csrf_token)) {
        http_response_code(403);
        echo json_encode(['error' => 'Session invalide (CSRF)']);
        exit;
    }
}
    ...
    switch ($action) {
        case 'get_users':
            try {
                ...
                foreach ($users as &$user) {
                    $user['created_at'] = date(DATE_FORMAT_HIDDEN, strtotime($user['created_at']));
                    $user['last_activity'] = $user['last_activity'] ? date(DATE_FORMAT_HIDDEN, strtotime($user['last_activity'])) : 'Jamais';
                }
        ...
        case 'get_table_details':
            try {
                ...
                $allowedTables = $tablesQuery->fetchAll(PDO::FETCH_COLUMN);
        ...
        case 'execute_sql':
            try {
                if ($_SESSION['user_role'] !== 'admin') {
                    throw new Exception('Accès réservé aux administrateurs pour cette action');
                }
